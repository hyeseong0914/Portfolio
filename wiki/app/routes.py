...
def edit(postid):
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            db.session.add(Post(title=title,body=body,user_id=user_id))
            db.session.commit()
            return redirect(url_for('index'))
        return render_template('edit.html', title='Edit Post', post=current_post, form=form)
    return render_template('notloggedin.html')
